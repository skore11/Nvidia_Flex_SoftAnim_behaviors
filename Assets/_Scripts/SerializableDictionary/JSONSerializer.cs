...
public class JSONSerializer
{
    ...
    public static string ToJson<T>(T data) where T : class
    {
        ...
        else if (typeof(T) == typeof(Dictionary<int, Vector3>))
        {
            return ToJsonIntVector3Dictionary(data as Dictionary<int, Vector3>);
        }
        else
        {
    ...
    [System.Serializable]
    private class IntVector3DictionaryArray
    {
        public IntVector3Dictionary[] items;
    }

    [System.Serializable]
    private class IntVector3Dictionary
    {
        public int key;
        public Vector3 value;
    }

    private static string ToJsonIntVector3Dictionary(Dictionary<int, Vector3> dictionary)
    {
        List<IntVector3Dictionary> dictionaryItemsList = new List<IntVector3Dictionary>();
        foreach(KeyValuePair<int, Vector3> kvp in dictionary)
            {
            dictionaryItemsList.Add(new IntVector3Dictionary() { key = kvp.Key, value = kvp.Value });
            }
        IntVector3DictionaryArray dictionaryArray = new IntVector3DictionaryArray() { items = dictionaryItemsList.ToArray() };
        return JsonUtility.ToJson(dictionaryArray);
    }
